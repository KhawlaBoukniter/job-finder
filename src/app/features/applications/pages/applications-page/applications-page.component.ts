          ...
          @for (app of applications; track app.id) {
            <div class="bg-white shadow rounded-lg p-6 border border-zinc-200">
              <div class="flex justify-between items-start">
                <div>
                  <h3 class="text-lg font-medium text-zinc-900">{{ app.title }}</h3>
                  <p class="text-sm text-zinc-500">{{ app.company }} • {{ app.location }}</p>
                </div>
                <span [ngClass]="{
              ...
              <div class="mt-4 flex space-x-3">
                 <a [href]="app.url" target="_blank" class="text-purple-600 hover:text-purple-800 text-sm font-medium">Voir l'offre original</a>
              </div>
            </div>
          }
        </div>
